...
func BenchmarkMimcCircuit(b *testing.B) {
	...
	mimcCircuit := AllocateGKRMimcTestCircuit(bN)
	// Attempt to compile the circuit
	r1cs, _ := frontend.Compile(ecc.BN254, r1cs2.NewBuilder, &mimcCircuit)

	fmt.Printf("Nb constraints = %v\n", r1cs.GetNbConstraints())

	// Generate the validAssignment values by running the prover
	var validAssignment GKRMimcTestCircuit
	...
	b.Run("Gnark circuit assignment", func(b *testing.B) {
		b.StopTimer()
		for i := 0; i < b.N; i++ {
			validAssignment = AllocateGKRMimcTestCircuit(bN)
			b.StartTimer()
			validAssignment.Assign(proof, inputs, outputs, qInitialprime)
			b.StopTimer()
		}
	})

	pk, _ := groth16.DummySetup(r1cs)
	witness, _ := frontend.NewWitness(&validAssignment, ecc.BN254)
